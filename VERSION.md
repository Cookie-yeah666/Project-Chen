...
## Unreleased
- 指引定位修复：屏幕目标定位前临时隐藏桌宠窗口并使用干净截图，避免 Vision 把桌宠气泡、分步指引面板、“我完成了/重新识别/退出”按钮误判成用户界面目标；普通聊天 API 未配置时，`.` 屏幕指示和分步指引不再被错误拦截。
- 中文词条/图标/按钮定位增强：扩大目标触发词，支持“设置图标”“下载字样”“Steam logo”等短目标请求；定位提示词区分汉语词条 OCR、图标/Logo 本体、按钮/链接/输入框整体控件，并兼容 `rect`、`box_2d`、`bbox_2d`、归一化坐标和候选列表返回。
- 下载指引增强：通用 fallback 会优先挑选可信官网/官方下载 URL，把网址写入步骤提示，配合指引面板点击复制；下载/安装步骤目标加入 Download、下载、Get、Install、Windows、Client、setup、installer 等可见候选词，减少泛化乱指。
- 屏幕目标定位算法 3：对中小文字、图标、按钮候选追加局部裁剪放大复核，Vision 在 crop 内重新确认目标中心和控件边界，再映射回全屏坐标；保留 8 秒高精度优先、失败后回退普通定位的主流程。
- 分步操作指引体验优化：主窗口顶部预留任务提示区，指引面板固定在人物上方独立区域，不再遮挡桌宠本体；教程提示中的 `https://` / `www.` 网址会渲染为可点击复制文本，用户点一下后可直接到浏览器 `Ctrl+V` 访问。
