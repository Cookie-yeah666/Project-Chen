...
- `operation-guide-progress-evaluator.ts`：分步指引的屏幕进度判断器；页面变化后用轻量 Vision JSON 判断当前步骤是否完成、下一步目标是否出现，避免只会重复指当前步骤。
...
- **桌宠可见性守护**：主窗口创建后启动 visibility watchdog，定期 `showInactive`、重新置顶、`moveTop`，并在窗口跑出屏幕时放回主屏幕；用于缓解打开 setup / installer 后普通置顶窗口被压下去的问题。`start.bat` 会自提权，Windows 打包配置在 `package.json > build.win.requestedExecutionLevel=requireAdministrator`；UAC 安全桌面期间无法显示，属于 Windows 限制。
